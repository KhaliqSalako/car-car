...
function SalesForm() {
  ...
  const getData = async () => {
    ...
    if (responseAutomobiles.ok && responseSalespersons.ok && responseCustomers.ok) {
  ...
  const handleDeleteAutomobile = async () => {
    ...
    const response = await fetch(`http://localhost:8100/api/automobiles/${formData.automobile}`, fetchConfig);

    if (response.ok) {
      let filterdAutomobiles = automobiles.filter(automobile => automobile.vin !== formData.automobile);
